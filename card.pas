...
type CardInfo = class
  ...
  imageMap : TDictionary<string,TImage>;

  private
  procedure initCardMap();
  procedure initBackCard();
  procedure initImageMap();
...
begin
   ...
   imageMap:=TDictionary<string,TImage>.Create;
...
procedure CardInfo.initImageMap();
var
  i: integer;
  im : TImage;
begin
  for i := 1 to 8 do
    begin
     im:=TImage.Create(GameInterface);
     im.Width:=GameInterface.bujiao.Width;
     im.Height:=GameInterface.bujiao.Height;
     im.Parent := GameInterface;
      if i<3 then
      begin
         im.Name:=GameInterface.buchu.Name+i.ToString;
         im.Bitmap.Assign(GameInterface.buchu.Bitmap);
         im.Visible:=false;
         ImageMap.TryAdd(im.Name,im);
      end;

      if (i>=3) and (i<5) then
      begin
        im.Name:=GameInterface.bujiao.Name+(i-2).ToString;
        im.Bitmap.Assign(GameInterface.bujiao.Bitmap);
         im.Visible:=false;
        ImageMap.TryAdd(im.Name,im);
      end;
      ...
      if (i>=5)and(i<7) then
      begin
        im.Name:=GameInterface.buqiang.Name+(i-4).ToString;
        im.Bitmap.Assign(GameInterface.buqiang.Bitmap);
         im.Visible:=false;
        ImageMap.TryAdd(im.Name,im);
      end;
      ...
      if (i>=7)and (i<8) then
      begin
        im.Name:=GameInterface.qiangdizhu.Name+(i-6).ToString;
        im.Bitmap.Assign(GameInterface.qiangdizhu.Bitmap);
         im.Visible:=false;
        ImageMap.TryAdd(im.Name,im);
      end;
    end;
...
procedure CardInfo.closeimages();
var
  im : Timage;
begin
